...
# Forecast Plot
def fun(x):
    d1 = final_df[[f'{x} HRC (FOB, $/t)']].loc["2024-03-01":]
...
with china_col:
    ...
    inland_c = st.number_input("Freight Port to City - China", value=500)
    landed_price_c = exchange_rate_c * (lc_port_c + 1.01 * (forecast_china + sea_freight_c) * (1 + 1.1 * duty_c)) + inland_c
    st.metric("China HRC Landed Price", f"{landed_price_c:,.0f}")
...
with jk_col:
    ...
    inland_j = st.number_input("Freight Port to City - Japan/Korea", value=500)
    landed_price_j = exchange_rate_j * (lc_port_j + 1.01 * (forecast_japan + sea_freight_j) * (1 + 1.1 * duty_j)) + inland_j
    st.metric("Japan/Korea HRC Landed Price", f"{landed_price_j:,.0f}")
